...
impl Plugin for WaterSurfaceVisualPlugin {
    fn build(&self, app: &mut App) {
        app.init_resource::<WaterSurfaceVisualCatalog>()
            ...
            .configure_sets(
                Update,
                WaterSurfaceVisualSet.in_set(super::TileWorldFallbackAfterFireExtract),
            )
...
fn mark_water_surface_motion_dirty(
    ...
    mut raster_ctrl: ResMut<super::TileWorldFallbackRasterCtrl>,
) {
    if !catalog.has_motion() {
        return;
    }
    ...
    const CHUNK: u32 = super::tile_world_fallback::RASTER_CHUNK_TILES;
