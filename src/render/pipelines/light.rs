...
//!
//! [`RequestLocalLight`](crate::render::pipelines::light::RequestLocalLight) messages (fire path from
//! [`FireVisualFrame`](crate::render::extraction::sim_visual_extract::FireVisualFrame)); this plugin **collects**, **scores**,
//! **sorts**, and **truncates** into [`ActiveLights`], then syncs a **fixed pool** of [`PointLight`]
//! entities (no per-frame spawn/despawn).
//!
//! Intended for:
//! - fire lighting via [`crate::render::extraction::sim_visual_extract::FireVisualFrame`] (single sim pass, then cluster emit)
