...
pub fn update_visual_agreement_frame(
    ...
    if let Some(graph) = projection.as_deref() {
        // VT4-WITNESS-001: `graph.fire.chunk_heat` is already LOD-binned (chunk_heat_bin>1 merges
        // NxN chunk cells). Comparing it against the raw unbinned `fire.chunk_heat` hash was a
        // permanent false positive at Operational/Macro LOD. Apply the SAME binning to the source
        // side before hashing — `agreement.fire_heat_hash` itself stays unbinned (field/JSON/log
        // stability), this is a comparison-local value only.
        let bin = graph.fire.chunk_heat_bin.max(1);
        let source_hash_for_comparison = if bin <= 1 {
            agreement.fire_heat_hash
        } else {
            hash_chunk_fire_heat(&bin_merge_chunk_heat(&fire.chunk_heat, bin))
        };
        if agreement.projected_fire_heat_hash != source_hash_for_comparison {
            agreement.mismatch_count = agreement.mismatch_count.wrapping_add(1);
            warn!(
                "VT-4 projected fire heat hash mismatch: frame={} (binned@{}={}) projection={}",
                agreement.fire_heat_hash, bin, source_hash_for_comparison, agreement.projected_fire_heat_hash
            );
        }
    }
...
mod tests {
    ...
    /// VT4-WITNESS-001: at `chunk_heat_bin>1` the projection LOD-merges chunk_heat cells before
    /// hashing. Binning the raw source with the SAME policy must reproduce the projected hash —
    /// proving the comparison is apples-to-apples (was a permanent false-positive VT-4 mask 0x4).
    #[test]
    fn binned_source_hash_matches_projected_hash_at_bin_two() {
        let raw_rows = vec![
            ChunkFireHeat { chunk: IVec2::new(0, 0), heat: 0.4, smoke: 0.0 },
            ChunkFireHeat { chunk: IVec2::new(1, 0), heat: 0.9, smoke: 0.2 },
            ChunkFireHeat { chunk: IVec2::new(0, 1), heat: 0.3, smoke: 0.0 },
            ChunkFireHeat { chunk: IVec2::new(1, 1), heat: 0.6, smoke: 0.1 },
            ChunkFireHeat { chunk: IVec2::new(2, 0), heat: 0.7, smoke: 0.0 },
        ];
        let bin = 2;
        let binned_source = bin_merge_chunk_heat(&raw_rows, bin);
        // Simulate what FireProjectionNode::evaluate produces at chunk_heat_bin=2: the projection
        // graph's chunk_heat IS the bin-merged list (project_chunk_heat delegates to the same fn).
        ...
        // And the raw (unbinned) hash must differ — proves binning actually changed the data,
        // i.e. the old direct-compare (raw vs binned) really was a guaranteed mismatch.
        assert_ne!(hash_chunk_fire_heat(&raw_rows), projected_hash);
    }

    /// End-to-end: `update_visual_agreement_frame` must not flag a VT-4 mismatch when the
    /// projection graph reports `chunk_heat_bin=2` and its `chunk_heat` is the correctly
    /// bin-merged version of the source frame's raw chunk heat.
    #[test]
    fn vt4_agreement_no_mismatch_when_projection_is_binned() {
        let stamp = SimStepStamp::new(5, 500);
        let raw_rows = vec![
            ChunkFireHeat { chunk: IVec2::new(0, 0), heat: 0.4, smoke: 0.0 },
            ChunkFireHeat { chunk: IVec2::new(1, 0), heat: 0.9, smoke: 0.2 },
            ChunkFireHeat { chunk: IVec2::new(0, 1), heat: 0.3, smoke: 0.0 },
        ...
        let mut graph = RenderProjectionGraph::default();
        graph.fire.chunk_heat_bin = 2;
        graph.fire.chunk_heat = bin_merge_chunk_heat(&raw_rows, 2);
        let mut agreement = VisualAgreementFrame::default();
        update_visual_agreement_frame(
        ...
        assert_eq!(
            agreement.mismatch_count, 0,
            "binned projection at chunk_heat_bin=2 must agree with the same-binned source, not the raw unbinned hash"
        );
        assert_ne!(
