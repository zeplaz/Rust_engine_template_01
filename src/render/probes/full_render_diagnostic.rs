...
/// RGR-M1-004: latch for full-render diagnostic capture systems (D3).
#[must_use]
pub fn full_render_diagnostic_plugin_latch() -> bool {
    crate::dev::test_run_instrumentation::diagnostics_operator_trace_enabled(
        false,
        &["FULL_RENDER_DIAG", "STAGE5_VERBOSE", "VISUAL_DIAG"],
    )
}
...
impl Plugin for FullRenderDiagnosticPlugin {
    fn build(&self, app: &mut App) {
        ...
        if !full_render_diagnostic_plugin_latch() {
            return;
        }
        app.add_systems(Startup, arm_full_render_diagnostic_for_full_app)
            .add_systems(
                Update,
