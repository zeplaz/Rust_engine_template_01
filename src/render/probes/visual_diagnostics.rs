...
use crate::gui::{
    CameraVisualState, MainWorldCameraOrthoTrace,
...
/// RGR-M1-004: latch for registering visual diag systems (D3).
#[must_use]
pub fn visual_diag_plugin_latch() -> bool {
    visual_diag_enabled(None)
}
...
fn log_visual_diag_snapshot(frame: u64, periodic: bool, ctx: &VisualDiagCtx) {
    let win_logical = ctx
        .windows
        .single()
...
pub(crate) fn trace_visual_diagnostics(
    ...
    log_visual_diag_snapshot(frame_n, periodic, &ctx);
...
impl Plugin for VisualDiagnosticsPlugin {
    fn build(&self, app: &mut App) {
        ...
        if !visual_diag_plugin_latch() {
            return;
        }
        app.add_systems(Startup, log_visual_diag_startup)
            .add_systems(
                Last,
