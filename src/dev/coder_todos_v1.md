...
**Date:** 2026-07-03 (session 2) · **Branch:** `master` · **Bevy 0.19** · **Migration:** CLOSED (`mig_a_program_close.json`)
...
### @operator / @coder_b (P0 — display required)
...
| ☐ | **DR-RTT-VR16** | `cargo run --release -- --test vfx` · refresh `stage5_full_app_live.json` |

| ☐ | **PERF-INSTR-VFX-002 accept** | Same run · clear `display_acceptance_pending` |
...
| ☐ | **BQ-K1 bake** | @coder-mcp | Wire K1 batch GLBs + style_pack slot targets per `bq_k1_kitfill_batch_v1.json` |
...
1. **GPU-P0C-PRIME** — [`gpu_todos_v1.md`](gpu_todos_v1.md) § PRIME-001..004 (@coder)  

2. **Operator P0** — VR16 + PERF display when machine available  

3. **APSR-P/D** (@coder-mcp) — panel split + design lint  

4. **BQ-K*** after designer charters  

5. **SCH-W2** planning — steward gate (`DR-SCHED-W2`)
...
cargo test -p proc_A_dine01 --lib sch_w1 cln_p0 bq_q3 building_quality stage5 -q
