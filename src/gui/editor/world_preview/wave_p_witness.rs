//! Wave P preview witness collectors + lib tests (DEV-CONTAIN-004).
//!
//! File I/O writer: [`crate::dev::runtime_witness::wave_p`].

use crate::gui::editor::world_preview::wave_p_readiness_passes;

pub const WAVE_P_LIVE_JSON: &str = "debug_runs/wave_p_live.json";

pub use crate::dev::runtime_witness::wave_p::{
    write_wave_p_witness_system, WavePLiveProofState,
};

#[must_use]
pub fn build_wave_p_witness_payload(
    ...
    let d02_present = d02.is_some();
    let d07_present = d07.is_some();
    ...
    body["ui_phase4_wp_tail_001"] = build_ui_phase4_wp_tail_witness(
        d02_present,
        d07_present,
        layout_002,
        layout_d07,
...
#[must_use]
pub fn build_ui_phase4_wp_tail_witness(
    d02_present: bool,
    d07_present: bool,
    layout_002_green: bool,
    layout_d07_green: bool,
    wave_p_green: bool,
) -> serde_json::Value {
    let optional_tail_green = d02_present && d07_present && layout_d07_green;
    serde_json::json!({
        "gate": "UI-PHASE4-WP-TAIL-001",
        "optional": true,
        "d02_present": d02_present,
        "d07_present": d07_present,
        "layout_002_green": layout_002_green,
        "layout_d07_green": layout_d07_green,
        "wave_p_green": wave_p_green,
        "green": optional_tail_green && wave_p_green,
    })
}
...
mod tests {
    use super::*;
    use bevy::prelude::*;
    use crate::gui::{gather_wave_p_readiness, PreviewLayers, PreviewPathAuthority};
    ...
    fn cod_b_wp_witness_001_green_requires_all_layout_and_wave_p_fields() {
        let authority = PreviewPathAuthority::default();
        let report = gather_wave_p_readiness(PreviewLayers::BIOME, &authority);
        let good = build_wave_p_witness_payload(
            &report,
        ...
        let bad_d07 = build_wave_p_witness_payload(
            &report,
            Some(build_d04_layout_witness(true, true, 520.0)),
    ...
    #[test]
    fn ui_phase4_wp_tail_001_optional_rolls_up_layout_and_wave_p() {
        let authority = PreviewPathAuthority::default();
        let report = gather_wave_p_readiness(PreviewLayers::BIOME, &authority);
        let body = build_wave_p_witness_payload(
            &report,
            Some(build_d04_layout_witness(true, true, 520.0)),
            Some(build_d07_layout_witness(true, d07_inset_side_px(), true)),
            Some(hd_baseline_d02_witness_sheet_closed()),
        );
        assert!(body["ui_phase4_wp_tail_001"]["optional"].as_bool().unwrap_or(false));
        assert!(body["ui_phase4_wp_tail_001"]["green"].as_bool().unwrap_or(false));
    }
