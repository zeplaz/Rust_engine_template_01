...
**Definition of done:**
- [ ] Build + test pass.
- [ ] `p4_hydrology.rs` is no longer an empty stub.
- [ ] `world_generator_plugin.rs` no longer contains no-op river/lake loops that imply real generation.
- [ ] No runtime hydrology behavior is invented beyond the signature/default scaffolding.
...
**Definition of done:**
- [ ] Build + test pass.
- [ ] River/lake entity spawning is separated from river/lake path computation.
- [ ] The transitional output is deterministic for a fixed seed.
- [ ] No new file beyond the single touch path is needed.
...
**Definition of done:**
- [ ] Build + tests pass.
- [ ] Same height grid + params produces byte-identical `HydrologyResult`.
- [ ] Greedy descent in `world_generator_enhanced.rs` is removed or routed through the new helper.
- [ ] Algorithm handles flat basins deterministically instead of random walking.
...
**Definition of done:**
- [ ] Build + test pass.
- [ ] Hydrology tags are written through `TagSet`; no stringly-typed runtime tags.
- [ ] Example tag registry contains the three hydrology tags.
- [ ] No p4 behavior depends on legacy ECS entity hierarchy.
...
**Definition of done:**
- [ ] Build + test pass.
- [ ] No random-walk or circle-only lake generation remains in `world_generator_enhanced.rs`.
- [ ] ECS visuals are a consumer of hydrology data, not a second hydrology model.
- [ ] `WorldGeneratorSubenginePlugin` remains legacy / out-of-scope per tooling rulebook.
...
**Definition of done:**
- [ ] Build + tests pass.
- [ ] G1 row Applied in the orchestrator.
- [ ] Terrain matrix p4 hydrology row Applied.
- [ ] Gap-hunt baseline no longer lists G1 hydrology as an open example.
