...
class LandscapeExtractParityPanel(ttk.LabelFrame):
    ...
    def refresh_parity(self) -> dict[str, Any]:
        ...
        if green:
            summary = (
                f"PASS — {body.get('authored_count')} authored veg keys resolve · "
                f"extract witness green"
            )
        else:
            missing = body.get("missing_from_resolver") or []
            summary = "FAIL — authored keys not consumable by engine resolver"
            if missing:
                summary += f" ({', '.join(missing[:3])}{'…' if len(missing) > 3 else ''})"
