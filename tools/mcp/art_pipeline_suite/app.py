...
from .aps_services import AssemblyService, AtlasService
...
from .aps_shell_wiring import wire_shell_events
...
class ArtPipelineSuiteApp(tk.Tk):
    def __init__(self) -> None:
        ...
        self.event_bus = EventBus(scheduler=lambda fn: self.after(0, fn))
        self.state_writer = SuiteStateWriter(self.event_bus)
        self.atlas_service = AtlasService(self.state, self.state_writer)
        self.assembly_service = AssemblyService(self.state, self.state_writer)
    ...
    def _apply_lane(self, lane: str, *, log: bool = True) -> None:
        ...
        previous_lane = self._applied_lane
        self._applied_lane = lane
        self.event_bus.publish("LaneChanged", {"lane": lane, "previous": previous_lane}, sync=True)
    ...
    def _build_buildings_tabs(self) -> None:
        job_kw = {
            ...
            "atlas_service": self.atlas_service,
    ...
    def _build_landscape_tabs(self) -> None:
        job_kw = {"start_job": self._start_job, "atlas_service": self.atlas_service}
    ...
    def on_send_to_assembly(self) -> None:
        if not self._show_flow_prerequisite("send_to_assembly"):
            return
        self.event_bus.publish("SendToAssembly", {}, sync=True)
        self.notebook.select(self.assembly._aps_tab_root)
    ...
    def on_bake_states(self) -> None:
        ...
        if expanded_batch.is_file():
            self.atlas_service.set_tile_batch_path(str(expanded_batch))
