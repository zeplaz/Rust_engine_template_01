...
## Role in three-track plan

| Global track | This doc |
|:---|:---|
| **Track B — Pipeline Spine** | Warehouse = **integration test** (“does spine work?”) |
| **Track A — APS Product** | Parallel — artist workflow ([`aps_authoring_tool_roadmap_v1.md`](aps_authoring_tool_roadmap_v1.md)) |
| **Track C — Content** | Parallel — grammar/content quality ([`plan_building_grammar_evolution_v1.md`](plan_building_grammar_evolution_v1.md)) |

**Not:** final production workflow definition. **Production sign-off** blocked until ARCH-MAT-001 + honest validators + preview clear ([`plan_three_track_execution_v1.md`](plan_three_track_execution_v1.md)).

## Two sub-tracks (warehouse pilot)

| Sub-track | What it proves | Status |
|:---|:---|:---:|
| **B1 — Grammar E2E** | Grammar snapshot → verify → preview → build (not placement-only) | **done** — [`pilot_grammar_001_grammar_e2e_live.json`](../../debug_runs/pilot_grammar_001_grammar_e2e_live.json) |
| **B2 — Ship E2E** | Manual keyframe → G4 → register | **blocked** — [`mcp_pilot_grammar_001_rejected_live.json`](../../debug_runs/art_pipeline/mcp_pilot_grammar_001_rejected_live.json) |

**Placement-only** = rejected for grammar proof.
...
**Paste prompts + queue drain:** [`pilot_grammar_agent_orders_v1.md`](pilot_grammar_agent_orders_v1.md)

```text
1. @coder-mcp  PILOT-GRAMMAR-E2E-001     grammar witness — DONE
2. @coder-mcp  APS-MAT-STUDIO-PHASE-A    Material Studio Phase A (Track B prep)
3. @coder-mcp  BUILD-WORKER-001          snapshot materials at assembly build (manual keyframe only for ship)
   parallel   PG-MODULE-AUDIT-002        P0 assembly_p0 green (corner/door production)
4. @designer-mcp MCP-PILOT-GRAMMAR-001  Phase 4–6 human keyframe + G4
5. @coder-mcp  register + @coder        Phase 7 (after proceed_ship: yes)
```
