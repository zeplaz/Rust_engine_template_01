...
| **Territory** | $ref:src/dev/plan_territory_matrix_002_v1.md — prefix → agent; §3 hotspots |
...
| **C** (downtime) | `src/systems/weather/`, weather witness writer, `ClimateState` (future), weather render extract | **All** of construction/, infrastructure/, tools/mcp/ — read hooks only via parallel lane §3.2 |
...
## Coder C — weather downtime queue

**Policy:** [`plan_weather_parallel_lane_v1.md`](plan_weather_parallel_lane_v1.md) · **Pull when** primary A/B `active[]` empty or dedicated Coder C assigned.

| # | ID | Files hint | Blocked by |
|:---:|:---|:---|:---|
| 1 | WEATHER-WITNESS-001 | `weather_sim_live.json` writer | witness-002 signed |
| 2 | WEATHER-CLIMATE-001 | `ClimateState`, `climate_slow_tick` | WEATHER-WITNESS-001 |
| 3 | WEATHER-REGIONAL-001 | regional → `ChunkWeather` | WEATHER-CLIMATE-001 |
| 4 | WEATHER-EFFECTS-001 | traction + visibility stubs | WEATHER-REGIONAL-001 |
| 5 | WEATHER-GPU-PRECIP-001 | GPU precip authority | WEATHER-EFFECTS-001 |

**Cross-system rule:** max **one** consumer file outside `systems/weather/` per PR; default impact = read scalars (renewables, visual, optional `weather_penalty`).
...
| WEATHER-* | CON-P2-* / INFRA-* / MCP | disjoint territories — weather never edits consumer modules |

---

## Current drain — construction only (2026-06-03)

**Authoritative:** [`construction_coder_drain_order_v1.md`](construction_coder_drain_order_v1.md)

| Coder | Order |
|:---|:---|
| **B** | ECON-OG-SAVE-001 → PROC-OG-APPROVE-001 → PROC-OG-POLICY-001 |
| **A** | PROC-OG-4-ROLLUP-001 → CON-P2-CLEARING-WIT-001 → PROC-PG-4-001 |

**P1–P6 closed on disk.** Infra E0–E6 / tile PT / weather — parallel lanes, not construction drain.
...
| Version | Date | Notes |
|:---|:---|:---|
| v1.2.0 | 2026-06-02 | Coder C weather downtime column; parallel lane policy |
