...
> Read [`docs/archive/2026-06-prompts-guides/runbooks/guides/gui_runbook_v1.md`](gui_runbook_v1.md) §§1-8, then open the active G3 sub-pack under `prompts/matrix/gap_remediation/runbook/`. Do not write Rust until the **slice** has a **§3 living map** row (or `ASK:` + **BQ-###**). Keep egui runtime placeholders marked `TEMP-EGUI`; desktop asset tools only edit files.
