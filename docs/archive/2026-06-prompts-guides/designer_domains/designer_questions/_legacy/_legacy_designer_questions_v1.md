...
>
> **Related:** `prompts/llm_agent_brief.md` · `prompts/README.md` · `docs/archive/2026-06-prompts-guides/matrix/matrix/README.md`
...
1. ~~**Genre + scale**: city-builder / RTS / sim / sandbox? Single map size target (km², tile count)?~~
   ...
   - **Scale (open):** map size in km² / tile count still TBD. Plan-for-chunking is locked (see Q9), so any single hard cap is a tooling concern, not a runtime ceiling.
     - **A1.a:** ~~target chunk size~~ → **⏳ provisional 256×256 tiles** — confirm in `docs/reference/designer_questions/terrain_world/chunks_streaming_v1.md`.
...
2. ~~**Player perspective**: 2D top-down, 2.5D iso, full 3D?~~
   ...
   - **A2.b:** tile sprite size / art pipeline — **tolerant of mixed quality & future expansion**; see `docs/reference/designer_questions/terrain_world/tile_sprites_v1.md`.
...
4. ~~**Save format**: JSON (current), RON, binary, mixed? Snapshot vs delta vs replay log?~~
   - **More efficient than JSON-everywhere.** Direction: binary snapshots + small RON / JSON metadata header; snapshot vs delta vs replay still TBD. **Matrix:** `docs/archive/2026-06-prompts-guides/matrix/matrix/serialization/serialization_hybrid_migration_matrix_v1.md`.
...
5. ~~**Tile model**: hard tiles vs continuous height/biome blend (`BiomeWeights`)? Both?~~
   - **Continuous blend.** Authoritative model is `BiomeWeights` + `TerrainSurfaceMix` (see `docs/archive/2026-06-prompts-guides/matrix/matrix/terrain_biome/terrain_biome_migration_matrix_v1.md`). Hard "TerrainClass" remains a derived/quantized view for rules/UI snapping.
6. ~~**Heightmap source of truth**: noise-only, or hand-edited overrides via tools?~~
   ...
   - **B6.b:** ~~brush set / undo~~ → **small initial set + expand later + undo chain** — `docs/reference/designer_questions/terrain_world/terrain_tools_brushes_v1.md`.
...
8. ~~**Rivers/lakes algorithm**: replace stub random walk with hydrology sim (flow/erosion)?~~
   ...
   - **B8.a:** **Worldgen = detailed hydrology.** **Runtime = lighter flow** + **deep sim on triggers** (dam breach, earthworks, flood/drain). Detail: `docs/reference/designer_questions/terrain_world/hydrology_v1.md`.
...
9. ~~**Streaming/chunking**: do we need partial worlds, or single in-memory map for prototype?~~
   ...
   - **B9.a–d:** **Chunk/streaming + sim partition + save granularity** — `docs/reference/designer_questions/terrain_world/chunks_streaming_v1.md`; **sim/render LOD** — `docs/reference/designer_questions/terrain_world/simulation_lod_v1.md`.
...
13. ~~**Power grid**: do we want full topology sim (substations/transformers/edges) or just zone-based supply?~~
   ...
   - **C13.a:** **Include frequency, AC vs DC, 1φ / 3φ** in model & **tech/cost progression** — full equation fidelity ⏳; see `docs/reference/designer_questions/production_economy/power_damage_ui_persistence_v1.md`.
   ...
   - **C13.d:** **Charts/graphs** (flows, power, resources over time) + **tiered, sortable, dismissible alerts** — `docs/reference/designer_questions/production_economy/power_damage_ui_persistence_v1.md`.
14. ~~**Damage/maintenance**: should `DamageState` tie into production, construction, vehicles?~~
   ...
   - **C14.c:** ~~split tracks?~~ → **One uber damage state** (wear + stress + combat); gradients & component breakages — `docs/reference/designer_questions/production_economy/power_damage_ui_persistence_v1.md`.
...
16. ~~**Pathfinding**: A* per-vehicle, flow fields, or hierarchical?~~ → **Hybrid hierarchical** (network + flow + refine; handoff by context + LOD). See `docs/reference/designer_questions/navigation/pathfinding_hierarchical_v1.md`.
17. ~~**Road graph**: separate ECS graph entity or terrain-tile annotations?~~
   ...
   - **D17.a:** **Rail ~ road graph abstraction but different rules** (signals, blocks, no passing) → separate type or heavy edge metadata; **optimize differently** — `docs/reference/designer_questions/navigation/pathfinding_hierarchical_v1.md`.
...
21. ~~**Faction count**: max 6 (current `FactionColors`) or dynamic?~~
   ...
   - **E21.a:** **Traits = placeholder**; expect **tag-oriented** flexibility — finalize in `docs/reference/designer_questions/production_economy/power_damage_ui_persistence_v1.md`.
...
22. **In-game HUD priorities**: resources, build menu, minimap, alerts — order? ⏳ Partial: **analytics charts + tidy alert feed** — see `docs/reference/designer_questions/production_economy/power_damage_ui_persistence_v1.md`.
...
26. ~~**Asset format per type:** vehicles → JSON, buildings → JSON, terrain → ?, shaders/textures → ?~~
   ...
   - **G26.a:** confirm RON vs JSON for hand-editable configs — track in `docs/archive/2026-06-prompts-guides/matrix/matrix/assets/bevy_asset_config_migration_matrix_v1.md`.
