...
## Intel officer — false-positive surveillance (supervised mutate)

**Only exception to read-only:** `intel_officer_apply` with `apply=true` after sweep review.

```text
node .claude/skills/agent-lang/driver.mjs intel-officer-sweep
node .claude/skills/agent-lang/driver.mjs intel-officer-apply --ids SLICE1,SLICE2        # dry-run preview
node .claude/skills/agent-lang/driver.mjs intel-officer-apply --ids SLICE1,SLICE2 --apply  # supervised reopen
node .claude/skills/agent-lang/driver.mjs intel-officer-sweep-witness
```

MCP: `intel_officer_sweep()` · `intel_officer_apply(ids, apply=true)`.

**Cull classes:** queue stale (exit_predicate / snag / contradiction) · done without witness · stub deliverables (TODO/TBD/short md) · SIGNED signoffs with missing/stub witness · dishonest `green:true` witnesses.

**Ritual:** sweep → ΔWF table with `recommended_action` per row → dry-run apply → operator OK → `--apply` → re-sweep until `green:true`. Never bulk-apply without listing slice_ids.
...
| **Intel officer** | false-green / stub-done detected | `intel_officer_sweep` → dry-run apply → supervised `--apply` reopen |
...
✅▶ intel_officer_apply --apply ONLY after sweep + dry-run + explicit slice_id list (supervised cull)
